//! Intel VT-x control flag helpers
...
impl PinCtls {
    ...
    pub fn read_raw() -> u64 {
        ...
        if (vmx_basic >> 55 & 1) == 1 {
            unsafe { Msr::new(VMCS_TRUE_PIN_BASED_VMEXEC_CTL).read() }
        } else {
    ...
    pub unsafe fn write_raw(value: u64) {
        ...
        if (vmx_basic >> 55 & 1) == 1 {
            Msr::new(VMCS_TRUE_PIN_BASED_VMEXEC_CTL).write(value)
        } else {
...
impl ProcCtls {
    ...
    pub fn read_raw() -> u64 {
        let vmx_basic = unsafe { Msr::new(IA32_VMX_BASIC).read() };
        if (vmx_basic >> 55 & 1) == 1 {
            unsafe { Msr::new(VMCS_TRUE_PROC_BASED_VMEXEC_CTL).read() }
        } else {
    ...
    pub unsafe fn write(flags: ProcCtlsFlags) {
        let old_value = Self::read_raw();
        let reserved = old_value & !(ProcCtlsFlags::all().bits());
        let new_value = reserved | flags.bits();
        Self::write_raw(new_value);
    }
    ...
    pub unsafe fn write_raw(value: u64) {
        let vmx_basic = Msr::new(IA32_VMX_BASIC).read();
        if (vmx_basic >> 55 & 1) == 1 {
            print!("TRUE PROC WRITE\n");
            Msr::new(VMCS_TRUE_PROC_BASED_VMEXEC_CTL).write(value);
        } else {
...
impl EntryCtls {
    ...
    pub fn read_raw() -> u64 {
        let vmx_basic = unsafe { Msr::new(IA32_VMX_BASIC).read() };
        if (vmx_basic >> 55 & 1) == 1 {
            unsafe { Msr::new(VMCS_TRUE_ENTRY_BASED_VMEXEC_CTL).read() }
        } else {
...
bitflags! {
    pub struct ExitCtlsFlags: u32 {
        ...
        /// CS.L, IA32_EFER.LME, and IA32_EFER.LMA on every VM exit. This control must be 0 on
...
impl ExitCtls {
    ...
    pub fn read_raw() -> u64 {
        let vmx_basic = unsafe { Msr::new(IA32_VMX_BASIC).read() };
        if (vmx_basic >> 55 & 1) == 1 {
            unsafe { Msr::new(VMCS_TRUE_EXIT_BASED_VMEXEC_CTL).read() }
        } else {
            unsafe { Msr::new(IA32_VMX_EXIT_CTLS).read() }
        }

        // unsafe { Msr::new(IA32_VMX_EXIT_CTLS).read() }
        // unsafe { rdmsr(IA32_VMX_EXIT_CTLS) }
    }
...
impl PageFaultErrorCode {
    ...
    pub fn is_bad_reserved_bit(&self) -> bool {
        self.value >> 3 & 1 == 1
    }
}
