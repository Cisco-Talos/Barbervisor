...
use crate::memreader::{Address, MemReader};
...
fn get_user_modules<R: MemReader>(
    gs_base: u64,
    cr3: u64,
    phys_file: &mut R,
    ...
    let peb = PEB::from_memreader(
        Address::Virtual(teb.process_environment_block, cr3),
        phys_file,
    );
    ...
    while curr_module != 0 {
        ...
        let ldr_data_entry =
            LdrDataTableEntry::from_memreader(Address::Virtual(curr_module, cr3), phys_file);
    ...
    let mut curr_module = ldr.in_memory_order_module_list.flink;
    while curr_module != 0 {
        // Get the current Ldr Entry
        let ldr_data_entry =
            LdrDataTableEntry::from_memreader(Address::Virtual(curr_module, cr3), phys_file);
        ...
        curr_module = ldr_data_entry.in_memory_order_module_list.flink;
    }
...
fn get_kernel_cr3<R: MemReader>(reader: &mut R) -> Option<u64> {
    ...
    for counter in (0x1aa000..0x1b0000).step_by(0x1000) {
        for i in (0..0x1000).step_by(8) {
            let curr_ptr = reader.read_u64(Address::Physical(counter + i)) & 0xffff_ffff_f000;
            if curr_ptr == counter {
...
fn get_kernel_modules<R: MemReader>(
    reader: &mut R,
    user_modules_hash: u64,
) -> HashMap<Range<u64>, String> {
    ...
    let kdbg = match File::open(&kdbg_file) {
        ...
        Err(_) => {
            ...
            'leave: loop {
                reader
                    .read_mem(Address::Physical(counter), &mut buffer)
                    .expect(&format!(
                        "Unable to read addr looking for KDBG: {:#x}",
                        counter
                    ));
                for i in 0..0x1000 - needle_len {
                    if &buffer[i..i + needle_len] == needle.as_slice() {
                        for back_offset in 0..0x40 {
                            ...
                            let possible_kdbg =
                                reader.read_u64(Address::Physical(check_addr)) & 0xffff_ffff_ffff;
                            let mut fileheader = [0; 2];
                            let res = reader.read_mem(
                                Address::Virtual(possible_kdbg, kernel_cr3),
                                &mut fileheader,
                            );
                            if res.is_err() {
                                continue;
                            ...
                            if fileheader == ['M' as u8, 'Z' as u8] {
                                ...
                                kdbg = possible_kdbg;
                                let mut file =
                                    File::create(kdbg_file).expect("Unable to create KDBG cache");
                                file.write_all(&kdbg.to_le_bytes())
                                    .expect("Failed to write KDBG cache");
                                break 'leave;
                            }
                        }
                    }
    ...
    let psloadedmodulelist_offset = match File::open(&plml_file) {
        Ok(mut file) => {
            let mut buffer = [0; 8];
            file.read_exact(&mut buffer)
                .expect("Failed to read old plml");
            u64::from_le_bytes(buffer)
        }
        Err(_) => {
            ...
            for offset in (0x0..0x1000_0000).step_by(8) {
                ...
                let psloadedmodulelist_phys =
                    reader.translate(Address::Virtual(psloadedmodulelist, kernel_cr3));

                // If translate_phys was null, continue to look
                if psloadedmodulelist_phys.is_none() {
                    continue;
                }
                ...
                let ldr_data_table_entry =
                    reader.read_u64(Address::Physical(psloadedmodulelist_phys)) & 0xffff_ffff_ffff;
                let ldr_data_table_entry_phys =
                    reader.translate(Address::Virtual(ldr_data_table_entry, kernel_cr3));

                // If translate_phys was null, continue to look
                if ldr_data_table_entry_phys.is_none() {
                    continue;
                }
                ...
                let data_entry = KldrDataTableEntry::from_memreader(
                    Address::Physical(ldr_data_table_entry_phys),
                    reader,
                );

                if data_entry.flink >> 63 == 0 {
                    continue;
                }
                if data_entry.blink >> 63 == 0 {
                    continue;
                }

                let curr_base = data_entry.base_dll(kernel_cr3, reader);
                if curr_base.is_none() {
                    continue;
                }
    ...
    assert!(
        psloadedmodulelist_offset > 0,
        "Failed to find PsLoadedModuleList offset"
    );
    // print!("PLML offset: {:x?}\n", psloadedmodulelist_offset);
    ...
    loop {
        let psloadedmodulelist_phys = reader
            .translate(Address::Virtual(psloadedmodulelist, kernel_cr3))
            .unwrap() as u64;
        let data_entry =
            KldrDataTableEntry::from_memreader(Address::Physical(psloadedmodulelist_phys), reader);
...
/// Given a `PDB`, return tuples of (AddressOfSymbol, SymbolName)
...
fn get_pdb(name: &str, hash: &str) -> File {
    ...
    let wsl_path = format!("_mnt_c_Symbols_{}_{}_{}", name, hash, name);
    if let Ok(file) = File::open(windows_path) {
        file
    } else {
        File::open(&wsl_path).expect(&format!("Unable to open PDB: {}", name))
    }
}

fn main() -> std::io::Result<()> {
    let mut phys_file =
        File::open("../snapshot/snapshot.phys").expect("FILE NOT FOUND: snapshot.phys");
    let mut file = File::open("../snapshot/SNAPSHOT_regs").expect("FILE NOT FOUND: snapshot_regs");
    let mut regs_data = Vec::new();
    file.read_to_end(&mut regs_data)
        .expect("Unable to read snapshot regs file");
    ...
    let user_modules = get_user_modules(gs_base, cr3, &mut phys_file);
    ...
    let kernel_cr3 = match File::open(&cr3_file) {
        ...
        Err(_) => {
            ...
            file.write_all(&kernel_cr3.to_le_bytes())
                .expect("Unable to write cr3 file");
            kernel_cr3
        }
    };
    ...
    let data = include_str!("../versions/win10.system32");
    ...
    for name in &all_pdbs {
        ...
        let pdb_hash = pdbs
            .get(pdb_name.as_str())
            .expect(&format!("Unable to get PDB: {}", name));
    ...
    let curr_rip = u64::from_str_radix(&std::env::args().nth(1).unwrap(), 16).unwrap();
    ...
    let (curr_cr3, other_cr3) = match curr_rip >> 63 & 1 {
        0 => (cr3, kernel_cr3),
        1 => (kernel_cr3, cr3),
        _ => unreachable!(),
    };

    let offset = curr_rip & 0xfff;
    let mut curr_instr_bytes = [0; 16];

    // Try both cr3s
    if phys_file
        .read_mem(Address::Virtual(curr_rip, curr_cr3), &mut curr_instr_bytes)
        .is_err()
    {
        print!("{:?}\n", curr_instr_bytes);
        phys_file.read_mem(Address::Virtual(curr_rip, other_cr3), &mut curr_instr_bytes);
        print!("{:?}\n", curr_instr_bytes);
    }
    let disasm = cs.disasm_count(&curr_instr_bytes, curr_rip, 1).unwrap();

    // Dump the found address
    print!("{} ", &rip);
...
impl VmRegs {
    ...
    pub fn print(&self) {
        unsafe {
            ...
            /*
            print!("xmm0: {:<#32x} xmm1: {:<#32x}\n", self.xmm0, self.xmm1);
            print!("xmm2: {:<#32x} xmm3: {:<#32x}\n", self.xmm2, self.xmm3);
            print!("xmm4: {:<#32x} xmm5: {:<#32x}\n", self.xmm4, self.xmm5);
            */
        }
    }
}
