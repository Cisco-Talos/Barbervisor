/// Taken from the smoltcp project.
///
...
impl fmt::Display for Protocol {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            &Protocol::Unknown(id) => write!(f, "0x{:04x}", id),
        }
    }
}
...
mod field {
    ...
    pub const PROTOCOL: Field = 12..14;
...
impl<T: AsRef<[u8]>> Frame<T> {
    ...
    /// Return the Protocol field, without checking for 802.1Q.
    #[inline]
    pub fn protocol(&self) -> Protocol {
        let data = self.buffer.as_ref();
        let raw = NetworkEndian::read_u16(&data[field::PROTOCOL]);
        Protocol::from(raw)
    }
}
...
impl<T: AsRef<[u8]> + AsMut<[u8]>> Frame<T> {
    ...
    pub fn set_protocol(&mut self, value: Protocol) {
        let data = self.buffer.as_mut();
        NetworkEndian::write_u16(&mut data[field::PROTOCOL], value.into())
    }
...
mod test_ipv4 {
    ...
    fn test_construct() {
        ...
        frame.set_protocol(Protocol::Ipv4);
...
mod test_ipv6 {
    ...
    fn test_construct() {
        ...
        frame.set_dst_addr(Address([0x01, 0x02, 0x03, 0x04, 0x05, 0x06]));
        frame.set_src_addr(Address([0x11, 0x12, 0x13, 0x14, 0x15, 0x16]));
        frame.set_protocol(Protocol::Ipv6);
